...
namespace MiniORM
{
	public abstract class DbContext
    {
        ...
        {
            var dbSets = this.dbSetProperties.Select(p => p.Value.GetValue(this)).ToArray();
        ...
        private void MapRelations<TEntity>(DbSet<TEntity> dbSet) where TEntity : class, new()
        {
            ...
            foreach (var collection in collections)
            {
                ...
                var mapColletionMethod = typeof(DbContext)
        ...
        private string[] GetEntityColumnNames(Type table)
        {
            ...
            var columns = table.GetProperties()
                .Where(p => dbColumns.Contains(p.Name) && !p.HasAttribute<NotMappedAttribute>() &&
