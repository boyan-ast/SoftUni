import { jsonRequester } from "../helpers/jsonRequester.js";

// Change the end of the url
let baseUrl = 'http://localhost:3030/data/movies';
let likesUrl = 'http://localhost:3030/data/likes';

async function getOne(id) {
    let result = await jsonRequester(`${baseUrl}/${id}`);
    return result;
}

async function getAll() {
    let result = await jsonRequester(baseUrl);
    return result;
}

async function create(item) {
    let result = await jsonRequester(`${baseUrl}`, 'POST', item, true);
    return result;
}

async function update(item, id) {
    let result = await jsonRequester(`${baseUrl}/${id}`, 'PUT', item, true);
    return result;
}

async function deleteOne(id) {
    let result = await jsonRequester(`${baseUrl}/${id}`, 'DELETE', undefined, true);
    return result;
}

async function getNumberOfLikes(movieId) {
    let result = await jsonRequester(`${likesUrl}/?where=movieId%3D%22${movieId}%22&distinct=_ownerId&count`);
    return result;
}

async function getUserLikesForMovie(movieId, userId) {
    let result = await jsonRequester(`${likesUrl}?where=movieId%3D%22${movieId}%22%20and%20_ownerId%3D%22${userId}%22`);
    return result;
}

async function addLike(item) {
    let result = await jsonRequester(`${likesUrl}`, 'POST', item, true);
    return result;
}

async function revokeLike(id) {
    let result = await jsonRequester(`${likesUrl}/${id}`, 'DELETE', undefined, true);
    return result;
}

export default {
    getAll,
    getOne,
    create,
    update,
    deleteOne,
    getNumberOfLikes,
    getUserLikesForMovie,
    addLike,
    revokeLike
}